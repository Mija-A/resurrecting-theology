...
GENERATION_MODEL_NAME = "falcon3-7b"
TOP_K = 1
...
def extract_number(text: str):
    ...
    # Cap out-of-range values — anything above 10 is not a valid WVS scale response
    if value > 10:
        return -99
...
def build_prompt(religion: str, scholar: str, question: str, coding: str) -> str:
    return (
        ...
        f"If unsure, make your best guess based on the context.\n"
        f"Question: {question}\n"
        f"Coding: {coding}\n"
        f"Answer:"
    )
...
def ask_model(prompt, retrieved_docs):
    ...
    full_prompt = (
        f"Retrieved context: {context_clean} "
        f"{prompt}"
    )
