...
Extract body text from:
  013 The Ultimate Meaning of Learning the Buddha-dharma.pdf
...
PDF_PATH = "/Users/mijasmacbookpro/Desktop/Prof Cantay Research/Buddhism Cleaned/Yin Shun/013 The Ultimate Meaning of Learning the Buddha-dharma.pdf"
OUT_PATH = "/Users/mijasmacbookpro/Desktop/Prof Cantay Research/Buddhism Cleaned/Yin Shun/013 The Ultimate Meaning of Learning the Buddha-dharma_clean.txt"
...
def find_footnote_cutoff(page):
    """
    Find where footnotes begin: first gap >= 50pt below 40% of page height.
    50pt avoids confusing paragraph spacing (~30pt) with footnote separators.
    """
...
def extract_body_text(page, cutoff_y):
    """
    Keep chars that are:
      - Font size >= 10.5pt (body text; excludes 9.1pt footnotes and 7pt superscripts)
...
def merge_into_paragraphs(lines):
    ...
    current = []
    for line in lines:
        is_heading = len(line) < 120 and bool(re.match(r'^(\d+\.|[A-Z][A-Za-z\s,\-]{5,}$)', line))
        if is_heading:
            if current:
