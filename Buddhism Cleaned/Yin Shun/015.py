"""
Extract body text from 015 The Core Tenet of the Buddha-dharma is Great Compassion.pdf
...
PDF_PATH = "/Users/mijasmacbookpro/Desktop/Prof Cantay Research/Buddhism Cleaned/Yin Shun/015 The Core Tenet of the Buddha-dharma is Great Compassion.pdf"
OUT_PATH = "/Users/mijasmacbookpro/Desktop/Prof Cantay Research/Buddhism Cleaned/Yin Shun/015 The Core Tenet of the Buddha-dharma is Great Compassion_clean.txt"
...
def find_footnote_cutoff(page):
    """
    Find where footnotes begin by looking for the first gap >= 50pt
    below 40% of the page height. Uses 50pt to avoid mistaking
    paragraph spacing (~30pt) for a footnote separator.
    """
...
def extract_body_text(page, cutoff_y):
    """
    Keep only chars that are:
      - Font size >= 10.5pt  (body; excludes 9.1pt footnotes and 7pt superscripts)
...
def merge_into_paragraphs(lines):
    """Join lines into paragraphs, insert blank lines before section headings."""
    HEADING_RE = re.compile(
        r'^(\d+\.|[A-Z][A-Za-z\s,\-]{5,80}$)'
    )
    paragraphs = []
    current = []
    for line in lines:
