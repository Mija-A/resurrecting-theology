...
pdf_path = "/Users/mijasmacbookpro/Desktop/Prof Cantay Research/Sri Aurobindo/hymns-to-the-mystic-fire.pdf"
output_path = "/Users/mijasmacbookpro/Desktop/Prof Cantay Research/Sri Aurobindo/hymns-to-the-mystic-fire_clean.txt"
...
for page_text in raw_pages:
    for line in page_text.split("\n"):
        s = line.strip()
        if not s:
            output_lines.append("")  # preserve blank lines for structure
            continue
        if is_page_number(s):
            continue
        if has_unicode(s):
...
# ── Step 4: Clean up excessive consecutive blank lines ────────────────────
cleaned = []
prev_blank = False
